...
def train_worker(rank, world_size, alpha, mu, dataset="CIFAR10", weight_decay=1e-4, max_epochs=30, spike_threshold=1.5, instability_std=0.2, switch_min_epoch=23, batch_size=128, lr=1e-3):
    ...
    for epoch in range(max_epochs):
        ...
        # --- Save checkpoint at epoch 34 ---
        if rank == 0 and epoch == 34:
            save_checkpoint(model.module.state_dict(), optimizer.state_dict(), "checkpoints/CIFAR100_epoch34_preARP.pth")
            print("💾 Midpoint checkpoint saved at epoch 34")
            break
