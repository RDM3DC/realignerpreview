# ------------------------------------------------------------
# realignr_ddp_simple_fixed.py — Simplified Windows-compatible DDP version 
# ------------------------------------------------------------
...
def main_worker(gpu_id, num_gpus, args):
    ...
    try:
        autocast_ctx = torch.amp.autocast('cuda') if use_amp else DummyContextManager()
    except:
        # Fallback for older PyTorch versions
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            if is_main_process and len(loss_history) >= PLATEAU_PATIENCE and step % PLATEAU_PATIENCE == 0:
                ...
                if slope > PLATEAU_SLOPE_THRESHOLD:  # Plateau detected
                    if isinstance(optimizer, ARPOptimizer):
                        # Try to access param_groups first since that's the most reliable approach
                        if optimizer.param_groups and len(optimizer.param_groups) > 0 and 'alpha' in optimizer.param_groups[0] and 'mu' in optimizer.param_groups[0]:
                            ...
                            print(f"Plateau detected at step {step}. Slope: {slope:.5f}. ARPOptimizer: α {old_alpha:.4f}→{optimizer.param_groups[0]['alpha']:.4f}, μ {old_mu:.4f}→{optimizer.param_groups[0]['mu']:.4f}")
                        ...
                        # Try direct attributes as fallback
                        elif hasattr(optimizer, 'alpha') and hasattr(optimizer, 'mu'):
                            ...
                            print(f"Plateau detected at step {step}, but couldn't update ARPOptimizer parameters (attributes not found)")
                            
                            if writer:
                                writer.add_text("Scheduler/plateau_event", f"Plateau detected but couldn't update optimizer params", step)
                    else:
                        print(f"Plateau detected at step {step}. Slope: {slope:.5f}. (Non-ARP optimizer)")
...
if __name__ == "__main__":
    ...
    if num_gpus > 1 and cuda_available:
        os.environ["MASTER_ADDR"] = "localhost"
        os.environ["MASTER_PORT"] = "12355"
        os.environ["WORLD_SIZE"] = str(num_gpus)
        
        # Windows-specific settings to avoid libuv errors
        os.environ["TORCH_DISTRIBUTED_DEBUG"] = "DETAIL"
        
    # Launch processes
    print(f"Starting training with {num_gpus} {'GPUs' if cuda_available else 'CPU worker(s)'}")
    
    # For Windows, let's use a single GPU for now to avoid DDP issues
    if os.name == 'nt' and cuda_available:
        print("Running on Windows - using single GPU mode for better compatibility")
