...
def train_worker(rank, world_size, alpha, mu, dataset="CIFAR10", weight_decay=1e-4, max_epochs=30, spike_threshold=1.5, instability_std=0.2, switch_min_epoch=23, batch_size=128, lr=1e-3):
    ...
    for epoch in range(max_epochs):
        ...
        if rank == 0:
            ...
            # --- Mid-run ARP CPR Protocol v2 ---
            if switched and epoch in [46, 60] and accuracy < 10.0:
                print(f"\n🧠 Emergency ARP recovery — accuracy too low at epoch {epoch}, reinitializing G_ij with noise\n")
                with torch.no_grad():
            ...
            if not switched and epoch >= switch_min_epoch:
                ...
                if delta_acc >= spike_threshold:
                    ...
                    abort_decision = True
            if epoch == 16: # Definite switch at epoch 35
                switch_decision = True
    ...
    # --- Mid-run ARP memory recovery with layered resuscitation ---
            if switched and epoch in [46, 60]:
                print(f"\n🧠 Mid-run ARP recovery trigger at epoch {epoch} — soft-resetting G_ij\n")
                with torch.no_grad():
                    for group in optimizer.param_groups:
                        for p in group['params']:
                            if p.grad is not None:
                                grad = p.grad.detach().abs()
                                state = optimizer.state[p]
                                state['G'] = (alpha / mu) * grad.clamp(min=1e-5, max=100.0)

            # Emergency ARP recovery if accuracy is too low
            if switched and accuracy < 10.0 and epoch in [46, 60]:
                print(f"\n🧠 Emergency ARP recovery — accuracy too low at epoch {epoch}\n")
                with torch.no_grad():
                    for group in optimizer.param_groups:
                        for p in group['params']:
                            if p.grad is not None:
