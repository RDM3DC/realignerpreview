...
# --- DDP Training Worker ---
def train_worker(rank, world_size, alpha, mu, dataset="CIFAR10", weight_decay=1e-4, max_epochs=30, spike_threshold=1.5, instability_std=0.2, switch_min_epoch=5, batch_size=128, lr=1e-3):
    ...
    model = DDP(model, device_ids=[rank], find_unused_parameters=False) # Set find_unused_parameters based on model structure
    ...
    test_loader = DataLoader(test_set, batch_size=batch_size * 2, shuffle=False, num_workers=4, pin_memory=True) # Larger batch for eval
...
def main():
    ...
    config = {
        ...
        "spike_threshold": 1.5,
        "instability_std": 0.2,
        "switch_min_epoch": 30, # Start checking for switch condition
    ...
    RUN_PRETRAINING = True # <-- Set this to True to run pre-training first
