...
def visualize_optimization(func, xlim, ylim, trajectories, title, save_path):
    ...
    valid_animation = all(any(not (math.isnan(p[0]) or math.isnan(p[1])) for p in traj) 
...
def optimize_function(func, x0, y0, optimizers, steps=100):
    ...
    for name, (optimizer_class, kwargs) in optimizers.items():
        ...
        final_values[name] = (model.xy.data.tolist(), loss.item())
        
        print(f"  Final position: xy = {model.xy.data.tolist()}, loss = {loss.item():.6f}")
...
def test_optimizers(save_dir='results/optimization_landscape'):
    ...
    optimizers = {
        ...
        'ARPPiA': (ARPPiAGradientDescent, {'lr': 0.01, 'alpha': 0.01, 'mu': 0.001}),
        'ARPPiA+': (ARPPiAGradientDescentPlus, {'lr': 0.01, 'alpha': 0.01, 'mu': 0.001, 
    ...
    for func_name in ["Rosenbrock", "Himmelblau", "Beale", "Rastrigin"]:
        for opt_name, (final_pos, final_loss) in final_values.items():
            print(f"{func_name:10} | {opt_name:8} | ({final_pos[0]:.4f}, {final_pos[1]:.4f}) | {final_loss:.6f}")
